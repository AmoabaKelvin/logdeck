...
import {
  removeContainerDemo,
  restartContainerDemo,
  startContainerDemo,
  stopContainerDemo,
} from "@/lib/logdeck-demo/demo-api";

import { useContainerStats, useContainersQuery, useSystemStats } from "./demo-hooks";
import { useContainersDashboardUrlState } from "./use-containers-dashboard-url-state";
...
import type { GetContainersResponse } from "@/lib/logdeck-demo/demo-api";
...
export function ContainersDashboard() {
  ...
  const matchesFilters = useMemo(() => {
    ...
    return (
      ...
      const matchesSearch =
        ...
        container.names.some((name) => name.toLowerCase().includes(normalizedSearch));
  ...
  const executeAction = async (
    ...
    try {
      ...
      switch (actionType) {
        case "start":
          message = await startContainerDemo(container.id);
          break;
        case "stop":
          message = await stopContainerDemo(container.id);
          break;
        case "restart":
          message = await restartContainerDemo(container.id);
          break;
        case "remove":
          message = await removeContainerDemo(container.id);
          break;
        default:
          return;
      }
      ...
      await queryClient.invalidateQueries({ queryKey: ["containers", "stats"] });
    } catch (error) {
  ...
  const handleContainerRecreated = async (newContainerId: string) => {
    ...
    const updatedData = queryClient.getQueryData<GetContainersResponse>(["containers"]);
