...
// The mutate function receives the current auth config (or a new empty one) and
// should return the modified config to persist, or an error to abort.
...
func (m *Manager) merge() (*Config, ConfigSources) {
	...
	// Auth — stored in file config, env handled by auth.NewService() directly.
	if m.envSnapshot.AuthSet {
